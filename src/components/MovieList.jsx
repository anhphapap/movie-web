...
const MovieList = ({ nameList, openModal, type, country, category, size }) => {
      ...
      try {
        if (type === "new") {
          ...
          do {
            ...
            var movieList = listResponse.data.items || [];
            movieList.map(async (movie) => {
              const detailResponse = await axios.get(
                `https://ophim1.com/phim/${movie.slug}`
              );
              movie.view = detailResponse.data.movie.view;
              topList.push(movie);
            });
            page++;
            var lastTime = new Date(movieList[23].modified.time);
          } while (lastTime >= oneWeekAgo);
          topList.sort((a, b) => b.view - a.view);
          setMovies(topList.slice(0, 10));
        } else {
          var filterList = [];
          var page = 1;
          while (filterList.length < size) {
            var listResponse = await axios.get(
              `https://ophim1.com/danh-sach/phim-moi-cap-nhat?page=${page}`
            );
            var movieList = listResponse.data.items || [];
            movieList.map(async (movie) => {
              ...
              if (
                (category !== "" &&
                  detailResponse.data.movie.category[0].name === category) ||
                (country !== "" &&
                  detailResponse.data.movie.country[0].name === country) ||
                (type === "trailer" &&
                  detailResponse.data.movie.episode_current === "Trailer")
              )
                filterList.push(movie);
              if (filterList.length >= size) return;
            });
            page++;
          }
    ...
    return (
      <div className="my-8 relative">
        ...
        >
          {movies.map((item) => (
            <div
              className="aspect-video bg-cover rounded-md group cursor-pointer mr-[3%]"
              style={{
